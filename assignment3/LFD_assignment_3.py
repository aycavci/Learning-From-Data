...
def custom_features(df):
    ...
    df['count_sentence'] = df.sentence.apply(lambda sen: count_sentence(sen))
    
    df['count_words'] = df.sentence.apply(lambda sen: len(nlp(sen)))
    
    df['adj_count'] = df.sentence.apply(lambda sen: count_adj(sen))
    
    df['entity_count'] = df.sentence.apply(lambda sen: count_named_entity(sen))
    
    df['sentence']= df.sentence.apply(lambda sen: spacy_pos(sen))
...
def best_svm(X_train, Y_train, X_test, Y_test, kernel="linear", C=1.0):
...
if __name__ == "__main__":
    ...
    if args.create_custom_features:
        ...
        df = pd.DataFrame([ids, X_full, Y_full]).transpose()
        df.columns = ['ID', 'sentence', 'label']
    
        #df_test  = read_corpus(args.testset)
        
        df = custom_features(df)
        
        v = TfidfVectorizer()
        x = v.fit_transform(df['sentence'])
        df_vect = pd.DataFrame(x.toarray(), columns=v.get_feature_names()) 
        
        df = df.drop(['sentence'], axis=1)
        df_full = pd.concat([df, df_vect], axis=1)
        
        Train = df_full.iloc[:len(X_train)]
        Test = df_full.iloc[len(X_train):len(X_train)+len(X_dev),:]
        ...
        clf = Pipeline([('cls', SVC(kernel="rbf", C=1.0))])
		...
		print("-----BEST MODEL-----")
		best_svm(X_train, Y_train, X_test, Y_test, C=5.0)
