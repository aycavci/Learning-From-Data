...
def split_test_set_predict(model, X_test, Y_test, encoder):
    """ Do predictions and measure accuracy on our own test set (that we split off train) """
    # Get predictions using the trained model
    Y_pred = model.predict(X_test)
    # Finally, convert to numerical labels to get scores with sklearn
    Y_pred = numpy.argmax(Y_pred, axis=1)
    labels_pred = [encoder.classes_[idx] for idx in Y_pred]
    # If you have gold data, you can calculate accuracy
    Y_test = numpy.argmax(Y_test, axis=1)
    labels_test = [encoder.classes_[idx] for idx in Y_test]
    write_to_file(labels_pred, "labels_pred.txt")
    write_to_file(labels_test, "labels_test.txt")
...
def main():
    ...
    if True:
        split_test_set_predict(model, X_test, Y_test, encoder)
